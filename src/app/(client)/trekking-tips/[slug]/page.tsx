...
async function BlogFeaturePhoto({ slug }: { slug: string }) {
  ...
  if (BlogDetails) {
    return (
      <Image
        src={BlogDetails.img_url}
        alt={BlogDetails.slug}
        height={1020}
        width={1020}
    ...
    <div className="max-w-5xl mx-auto">
      <div className="pt-2 pr-0 pb-0 pl-0">
        {BlogDetails.category.map((cat) => (
          <a
            className="inline text-base font-medium mt-0 mr-1 mb-0 ml-0 bg-primary dark:bg-slate-200 px-2 py-1 rounded-full dark:text-black"
            key={cat}
          >
            {cat}
          </a>
      ...
      <div className="pt-2 pr-0 pb-0 pl-0 mt-10">
        <a className="inline text-base font-medium mt-0 mr-1 mb-0 ml-0 underline">
          {BlogDetails.authors}
        </a>
        <p className="inline text-base font-medium mt-0 mr-1 mb-0 ml-1">
          · {moment.utc(BlogDetails?.publishedAt).local().format("h:mm a")}·
        </p>
      </div>
      <h1 className="text-5xl font-bold tracking-wider my-10">
        {BlogDetails.title}
      </h1>
      <PortableText value={BlogDetails.body} components={RichtextComponents} />
    </div>
  );
}
